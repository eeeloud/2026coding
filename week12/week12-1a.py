# week12-1a.py 學習計劃 Graphs - DFS 第1題 Medium 題
# LeetCode 841. Keys and Rooms
# 房間裡有鑰匙, 可以再開新的房間。 最後能開全部房間嗎?
# DFS: Depth First Search 通常會利用 stack 或 function stack(函式呼叫函示)
class Solution:
    def canVisitAllRooms(self, rooms: List[List[int]]) -> bool:
        stack = [0] # 我們利用 stack 裡面有待處理的房間, 一開始 房間 0 是開的
        ...
        while stack: # 只要 stack 還有東西, 就繼續處理
            ...
            for k in rooms[now]: # 把 room now 房間裡, 所有的鑰匙 k, 都拿來檢查
                if k in visited: continue # 鑰匙 k 對應房間 k 看過了, 別再檢查
